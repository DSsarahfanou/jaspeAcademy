...
export default function Equipements() {
  ...
  //     console.log('Order and invoice created successfully:', res.data);
  //     toast.success('Paiement réussi ! Votre facture a été téléchargée.');

  //   } catch (error) {
  //     console.error('Error creating order:', error);
  //     alert('Paiement réussi, mais une erreur est survenue lors de la création de la commande.');
  //   }

  //   setPanier([]);
  //   setPanierOuvert(false);
  //   removeKkiapayListener('success', handleSuccess);
  //   removeKkiapayListener('error', handleSuccess);
  //   removeKkiapayListener('cancel', handleSuccess);
  // };
  ...
  const handleSuccess = async (response) => {
    ...
    try {
      ...
      //       >
      //         Cliquez ici pour voir/télécharger votre facture
      //       </a>
      //     </div>,
      //     {
      //       position: "top-right",
      ...
      //   );
      // } else {
      //   Toaster.success('Paiement réussi, mais facture indisponible.');
      // }
    ...
    if (invoiceUrl) {
      ...
      toast.success('Paiement réussi, mais facture indisponible.');
    }


    } catch (error) {
      console.error('Error creating order:', error);
      toast.error('Paiement réussi, mais une erreur est survenue lors de la commande.');
    }

    setPanier([]);
    setShowSuccessAnim(true);
    setTimeout(() => setShowSuccessAnim(false), 60000);
    setPanierOuvert(false);
    removeKkiapayListener('success', handleSuccess);
    removeKkiapayListener('error', handleSuccess);
    removeKkiapayListener('cancel', handleSuccess);
  ...
  const openPayment = () => {
    ...
    addKkiapayListener('error', (error) => {
      ...
      removeKkiapayListener('cancel', this);
    });
  ...
  <Toaster 
    ...
    toastOptions={{
      ...
      className: 'animate-fade-in-out', // Animation Tailwind
    }}
  />
  {showSuccessAnim && (
    <motion.div
      ...
      <div className="bg-white border border-green-200 rounded-xl shadow-xl overflow-hidden w-80">
        <div className="bg-gradient-to-r from-green-400 to-green-500 p-4">
          <div className="flex items-center">
            <div className="flex-shrink-0 text-white text-2xl">🎉</div>
            <div className="ml-3">
              <h3 className="text-lg font-bold text-white">Succès !</h3>
            </div>
          </div>
        </div>
        <div className="p-4">
          <p className="text-gray-800">Votre paiement a été effectué avec succès.</p>
          <div className="mt-4">
            <a
              href={url}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center w-full px-4 py-2 bg-green-50 text-green-700 rounded-md hover:bg-green-100 transition-colors duration-200"
            >
